...
function highlightMetrics(text: string): React.ReactNode {
  const metricPattern = /(\$?\d+[KkMm]?\+?%?x?|\d+\+?\s*(?:years?|months?|ARR|clients?))/gi;
    ...
    <section id="projects" className="bg-secondary/30 py-20 md:py-28">
      <div className="container mx-auto max-w-5xl px-6">
        ...
        <div className="relative">
          ...
          <div className="absolute left-6 top-0 hidden h-full w-px bg-gradient-to-b from-accent/50 via-border to-transparent md:left-8 md:block" />
...
function TimelineItem({ project, index, isLast }: TimelineItemProps) {
  ...
  return (
    <div
      ref={ref}
      className={cn(
        "relative grid gap-4 md:grid-cols-[64px_1fr] md:gap-8",
        "opacity-0",
    ...
    >
      ...
      <div className="hidden md:flex md:flex-col md:items-center">
        ...
        {!isLast && <div className="mt-4 h-2 w-2 rounded-full bg-border" />}
      </div>
          ...
          <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
            <div className="space-y-1">
              ...
              <div className="flex items-center gap-2">
                ...
                <h3 className="font-semibold text-foreground">{project.company}</h3>
              </div>

              {/* Role */}
              <p className="text-lg font-medium text-foreground/80">{project.role}</p>
            </div>
          ...
          <ul className="space-y-2">
            {project.impact.map((item, i) => (
              <li key={i} className="flex items-start gap-3 text-sm">
                ...
                <span className="text-foreground/90">{highlightMetrics(item)}</span>
              </li>
            ))}
          </ul>
